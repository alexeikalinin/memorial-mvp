# Локальная разработка и деплой (vspomin.ai)
...
### Локальная разработка с той же БД, что и на вебе (Supabase)

Да: в **`backend/.env`** укажите **`DATABASE_URL`** со строкой подключения к **PostgreSQL Supabase** (pooler), ту же самую, что задана на хостинге бэкенда (Railway/Render и т.д.). Тогда `uvicorn`, тесты и сиды с `DATABASE_URL` из окружения будут писать **в облако**, а не в `memorial.db`.

**Рекомендация:** для повседневной разработки лучше **отдельный** проект Supabase (staging) или отдельная схема/БД, чтобы эксперименты и `seed_english_all.py` не ломали продакшен-данные. Для продакшена — только осознанные миграции/сиды.

**Ограничения паритета:**

- **Медиа:** при `USE_S3=false` файлы остаются в `backend/uploads/` локально; на проде при `USE_S3=true` картинки в Storage — для полного совпадения включите те же S3/Supabase Storage ключи локально или используйте staging с тем же режимом.
- **Фронт:** `VITE_API_URL` на проде должен указывать на **тот же бэкенд**, который видит ту же БД (или общий staging API).
- **Секреты:** строку Supabase в git не коммитить; только в `.env` и в переменных сервиса деплоя.

Загрузка конфига: [backend/app/config.py](backend/app/config.py) — всегда читает `backend/.env`.

**EN-демо (43 мемориала):** одинаковые данные локально и на проде — только через один и тот же сид и одну БД; файл `*.db` в git не хранится. См. [docs/DEMO_PARITY.md](docs/DEMO_PARITY.md), проверка графа: `backend/verify_en_demo_graph.py`, сценарий пересева: `backend/scripts/rebuild_en_demo.sh`.
