export default {
  auth: {
    login_error: 'Invalid email or password',
    register_error: 'Registration failed. Please try again.',
    generic_error: 'Something went wrong. Please try again.',
    signin_title: 'Sign In',
    create_account_title: 'Create Account',
    password_updated: 'Password updated successfully. Sign in with your new password.',
    continue_google: 'Continue with Google',
    or_signin_email: 'or sign in with email',
    or_register_email: 'or register with email',
    email: 'Email',
    email_placeholder: 'your@email.com',
    password: 'Password',
    password_min_placeholder: 'Minimum 8 characters',
    username: 'Username',
    username_placeholder: 'username',
    full_name: 'Full name',
    full_name_placeholder: 'John Smith',
    optional: 'optional',
    signing_in: 'Signing in…',
    creating_account: 'Creating account…',
    forgot_password: 'Forgot password?',
    no_account: "Don't have an account?",
    signup_link: 'Sign up',
    have_account: 'Already have an account?',
    signin_link: 'Sign in',
    or: 'or',
    explore_demo: 'Explore Demo Families (no sign up)',
    check_email_title: 'Check your email',
    check_email_sub_1: 'If',
    check_email_sub_2: "is registered, you'll receive a password reset link shortly.",
    check_email_hint: "Didn't get it? Check your spam folder or try again in a few minutes.",
    back_to_login: '← Back to login',
    forgot_password_title: 'Forgot password?',
    forgot_password_sub: "Enter your email and we'll send you a reset link.",
    sending: 'Sending…',
    send_reset_link: 'Send Reset Link',
    invalid_link_title: 'Invalid link',
    invalid_link_sub: 'No reset token found. Please request a new password reset.',
    request_reset: 'Request Reset',
    passwords_mismatch: 'Passwords do not match.',
    password_too_short: 'Password must be at least 8 characters.',
    reset_failed: 'Reset failed. The link may be expired.',
    set_new_password_title: 'Set new password',
    set_new_password_sub: 'Choose a strong password for your account.',
    new_password: 'New Password',
    password_min_placeholder_short: 'At least 8 characters',
    confirm_password: 'Confirm Password',
    repeat_password: 'Repeat password',
    saving: 'Saving…',
    set_new_password_btn: 'Set New Password',
  },
  nav: {
    brand: 'vspomin.ai',
    home: 'Home',
    create: 'Create Memorial',
    logout: 'Log out',
    footer: '© 2026 vspomin.ai — your memory. forever.',
    page_title: 'vspomin.ai — your memory. forever.',
  },
  not_found: {
    title: 'Page not found',
    subtitle: "The page you're looking for doesn't exist or has been moved.",
    back: '← Go back',
  },
  public: {
    not_found: 'Memorial not found',
    qr_error: 'Error generating QR code',
    request_error: 'Error sending request',
    memory_submit_error: 'Failed to submit memory',
    loading: 'Loading...',
    demo_banner: 'This is a demo memorial.',
    demo_all_families: '← All families',
    demo_create_own: 'Create your own →',
    nav_chat: 'Chat with Avatar',
    nav_memories: 'Memories',
    nav_photos: 'Photos',
    anon_hint_full: '{limit} free questions · sign in for 15/month',
    anon_hint_left: '{left} of {limit} free questions left · ',
    anon_hint_link: 'sign up free for 15/month',
    anon_limit_title: "You've used all {limit} free questions",
    anon_limit_sub_1: 'Create a free account —',
    anon_limit_sub_2: '15 messages/month',
    anon_limit_sub_3: 'Upgrade to Plus for',
    anon_limit_sub_4: '200 messages/month + voice & photo animation',
    signup_free: 'Sign up free',
    signin: 'Sign in',
    no_memories: 'No memories have been shared yet.',
    memory_thanks: 'Thank you! Your memory has been sent to the owner for review.',
    share_another: 'Share another',
    share_memory: 'Share a memory',
    memory_review_hint: 'Your submission will be reviewed by the memorial owner before appearing here.',
    your_name: 'Your name *',
    title_optional: 'Title (optional)',
    your_memory: 'Your memory *',
    sending: 'Sending…',
    send_for_review: 'Send for review',
    cancel: 'Cancel',
    share_qr: 'Share QR Code',
    qr_generating: 'Generating QR code...',
    qr_hint_1: 'Scan the QR code or share the link.',
    qr_hint_2: 'Print and place on the memorial stone.',
    link_copied: 'Link copied!',
    copy: 'Copy',
    download_png: 'Download PNG',
    access_request_sent: 'Access request sent — waiting for owner approval',
    no_access_hint: "You don't have management access to this memorial",
    request_access: 'Request access',
    memory_page: 'Memory page',
    manage_memorial: 'Manage memorial →',
  },
  home: {
    memorial_candle_title: 'Memorial',
    label: 'vspomin.ai',
    tagline_plain: 'We preserve the person,',
    tagline_em: 'not just the photographs',
    subtitle: 'Create a lasting digital memory of your loved one — with voice, stories and a family tree',
    cta: 'Create Memorial',
    section_title: 'Memorials',
    count_label: '{n} memory pages',
    card_counts: '{memories} memories · {media} files',
    add_memories: 'Add memories',
    empty: 'No memorials created yet',
    create_first: 'Create your first memorial',
    show_demo: 'Show demo memorials',
    explore_demo: 'Explore demo families →',
    demo_hint:
      'English demo pages from the repo seed set are listed here when expanded — same data as in production, tucked away for a cleaner home list.',
    pricing_title: 'Simple, transparent pricing',
    pricing_subtitle: "Start free. Upgrade when you're ready.",
    pricing_cta: 'Get started free',
    pricing_popular: 'Most popular',
    pricing_lifetime_badge: 'One-time',
    pricing_mo: '/mo',
    pricing_yr: '/yr',
    pricing_plans: [
      {
        id: 'free',
        name: 'Free',
        price: '$0',
        period: '',
        desc: 'Try it out',
        features: [
          '1 memorial page',
          '15 chat messages / month',
          'Text chat with AI avatar',
          'Photo & video gallery',
          'Family tree',
        ],
        cta: 'Start free',
        highlight: false,
      },
      {
        id: 'plus',
        name: 'Plus',
        price: '$7',
        period: '/mo',
        annual: '$59/yr',
        desc: 'For families',
        features: [
          'Up to 10 memorial pages',
          '200 chat messages / month',
          'AI voice (TTS)',
          '5 photo animations / month',
          'Family RAG (cross-memorial chat)',
          '5 GB storage',
        ],
        cta: 'Start Plus',
        highlight: true,
      },
      {
        id: 'pro',
        name: 'Pro',
        price: '$15',
        period: '/mo',
        annual: '$120/yr',
        desc: 'For large families',
        features: [
          'Up to 10 memorial pages + add-ons',
          '500 chat messages / month',
          'AI voice (TTS)',
          '15 photo animations / month',
          'Family RAG',
          '5 live avatar sessions / month',
          '15 GB storage',
        ],
        cta: 'Start Pro',
        highlight: false,
      },
      {
        id: 'lifetime',
        name: 'Lifetime',
        price: '$99',
        period: '',
        desc: 'One memorial, forever',
        features: [
          '1 memorial (locked)',
          '200 chat messages / month',
          'AI voice (TTS)',
          '5 photo animations / month',
          '5 GB storage',
          'No subscription — pay once',
        ],
        cta: 'Buy Lifetime',
        highlight: false,
        badge: 'One-time',
      },
      {
        id: 'lifetime_pro',
        name: 'Lifetime Pro',
        price: '$249',
        period: '',
        desc: 'Live avatar, forever',
        features: [
          '1 memorial (locked)',
          '200 chat messages / month',
          'AI voice (TTS)',
          '15 photo animations / month',
          'Pool of 100 live avatar sessions',
          '10 GB storage',
          'No subscription — pay once',
        ],
        cta: 'Buy Lifetime Pro',
        highlight: false,
        badge: 'One-time',
      },
    ],
  },
  tabs: {
    media: 'Media',
    memories: 'Memories',
    chat: 'Chat with Avatar',
    family: 'Family Tree',
    timeline: 'Timeline',
  },
  lifeTimeline: {
    loading: 'Loading timeline…',
    empty_title: 'No memories yet',
    empty_hint:
      'Add memories — undated ones appear under "No date". Dated events sort by "When it was".',
    title: 'Life timeline',
  },
  chat: {
    placeholder: 'Ask a question...',
    send: 'Send',
    suggested_label: 'Try asking:',
    audio_label: 'Generate audio',
    family_label: 'Include family memories',
    no_info: "I don't have memories about that.",
    sources: 'Sources:',
    sources_toggle: 'Sources ({count})',
    questions: [
      'Tell me about your childhood',
      'What were your hobbies and passions?',
      'Tell me about your family',
      'What mattered most to you in life?',
      'What advice would you give your children?',
    ],
    title_with: 'Chat with {name}',
    title_default: 'Chat with AI avatar',
    sync_family: 'Sync family',
    syncing: 'Syncing…',
    fullscreen: 'Fullscreen',
    exit_fullscreen: 'Exit fullscreen',
    clear_history: 'Clear history',
    voice_clone: 'Clone avatar voice',
    voice_uploaded: 'Avatar voice loaded',
    voice_change: 'Change',
    voice_panel_title: 'Avatar voice clone',
    voice_hint: "The avatar will speak in this person's voice. At least 1 minute of clean speech without noise.",
    mic_denied: 'Microphone access denied.',
    voice_file_type_error: 'Please select an audio file (MP3, WAV, M4A)',
    voice_clone_success: 'Voice cloned successfully!',
    voice_clone_error: 'Error cloning voice',
    family_tooltip_title: 'Hint',
    family_tooltip_on: "When enabled, answers use memories from this person and from relatives' memorials where they are mentioned.",
    family_tooltip_off: "When disabled, only this person's memories are used.",
    family_locked_tooltip: 'Family memories across linked memorials is a Plus feature. Upgrade to unlock.',
    family_upgrade_prompt: '🔒 Family RAG is available on Plus and Pro plans. Upgrade at /#pricing to chat across linked memorials.',
    sync_confirm: "Run family memory sync? This may create new memories on relatives' memorials based on this text.",
    sync_done: 'Sync complete. Created: {created}, skipped: {skipped}.',
    sync_error: 'Sync failed',
    clear_confirm: 'Clear chat history?',
    chat_error: 'Failed to send message',
    tts_quota_on: 'ElevenLabs: ~{remaining} / {limit} characters left this period (tier: {tier}).',
    tts_quota_unlimited: 'ElevenLabs: usage reporting unavailable or unlimited for this plan.',
    tts_quota_off: 'Voice generation (ElevenLabs) is not configured on the server.',
    tts_quota_err: 'Could not load ElevenLabs quota.',
    welcome_intro: 'Ask a question about this person, and the AI avatar will answer based on the added memories.',
    video_generating: 'Generating video…',
    browser_no_video: 'Your browser does not support video.',
    browser_no_audio: 'Your browser does not support audio.',
    audio_play: 'Play',
    audio_pause: 'Pause',
    audio_seek: 'Playback position',
    audio_more: 'More options',
    audio_download: 'Download',
    audio_speed: 'Playback speed',
    audio_failed: 'Audio not generated:',
    voice_cloning: 'Cloning…',
    voice_use_recording: 'Use this recording',
    voice_add_sample: 'Add sample',
    voice_recording_label: 'Recording {n}',
    voice_samples_count: 'Voice samples: {n} (more clean recordings improve the clone)',
    voice_remove_sample: 'Remove sample',
    voice_clone_submit: 'Clone voice ({n})',
    voice_rerecord: 'Re-record',
    voice_or: 'or',
    voice_upload_label: 'Upload a recording',
    voice_upload_hint: '(if a recording was saved)',
    voice_choose_file: 'Choose file',
    voice_record_start: 'Start recording',
    voice_record_stop: 'Stop',
    rec_indicator: '● REC',
    voice_name_placeholder: 'Voice name (optional)',
    voice_record_now: 'Record now',
    voice_if_alive: '(if the person is alive)',
    voice_panel_close: 'Close',
    avatar_thinking: 'Thinking…',
    avatar_ready: 'Ready',
  },
  memorial: {
    loading: 'Loading...',
    not_found: 'Memorial not found',
    talk_to_avatar: 'Talk to Avatar',
    memories_section: 'Memories',
    photos_section: 'Photos',
    no_memories: 'No memories added yet',
  },
  detail: {
    error_load: 'Failed to load memorial',
    delete_confirm: 'Delete memorial "{name}"? This cannot be undone.',
    delete_error: 'Failed to delete memorial',
    approve_memory_error: 'Failed to approve memory',
    reject_memory_confirm: 'Delete this memory permanently?',
    reject_memory_error: 'Failed to reject memory',
    qr_error: 'Failed to load QR code. Ensure qrcode[pil] is installed.',
    cover_error: 'Failed to set cover photo',
    update_error: 'Failed to update memorial',
    invite_create_error: 'Failed to create invite link',
    revoke_invite_confirm: 'Revoke this link? It will stop working.',
    revoke_invite_error: 'Failed to revoke link',
    access_approve_error: 'Failed to approve request',
    access_reject_error: 'Failed to reject request',
    access_grant_error: 'Failed to grant access',
    access_role_error: 'Failed to change role',
    access_revoke_confirm: 'Revoke access for this user?',
    access_revoke_error: 'Failed to revoke access',
    btn_edit: 'Edit',
    btn_qr: 'QR code',
    btn_invite: 'Invite',
    btn_access: 'Access',
    btn_delete: 'Delete',
    deleting: 'Deleting…',
    edit_title: 'Edit memorial',
    label_name: 'Name *',
    label_description: 'Description',
    label_birth: 'Birth date',
    label_death: 'Death date',
    public_memorial: 'Public memorial',
    voice_avatar: 'Avatar voice',
    voice_unspecified: 'Not specified',
    voice_male: 'Male voice',
    voice_female: 'Female voice',
    save: 'Save',
    saving: 'Saving…',
    cancel: 'Cancel',
    qr_modal_title: 'Memorial QR code',
    qr_generating: 'Generating QR code…',
    qr_hint: 'Scan to open the public memorial page.<br />You can print it for a memorial site.',
    copy: 'Copy',
    download_png: 'Download PNG',
    link_copied: 'Link copied!',
    access_title: 'Memorial access',
    access_email: 'User email',
    access_role: 'Role',
    role_viewer: 'Viewer',
    role_editor: 'Editor',
    grant_access: 'Grant access',
    granting: 'Granting…',
    access_requests: 'Access requests',
    loading_short: 'Loading…',
    approve: 'Approve',
    reject: 'Reject',
    users_with_access: 'Users with access',
    no_access_users: 'No users with access',
    owner: 'owner',
    revoke: 'Revoke',
    invite_title: 'Invite a relative',
    invite_name_label: 'Invitee name (optional)',
    invite_placeholder: 'e.g. Dad, Aunt Mary',
    create_link: 'Create link',
    creating: 'Creating…',
    link_created: 'Link created:',
    active_links: 'Active links',
    no_links: 'No active links',
    uses_count: '{n} visits',
    no_name: 'No name',
    validation_field: 'field',
    date_placeholder: 'yyyy-mm-dd',
    date_invalid: 'Enter a valid date: yyyy-mm-dd or dd.mm.yyyy',
  },
  memorialCreate: {
    title: 'Create Memorial',
    subtitle: 'Preserve the memory of a loved one forever',
    name_placeholder: "Person's name",
    description_placeholder: 'Short description or biography',
    gender_label: 'Gender *',
    gender_placeholder: 'Select gender',
    gender_hint: 'Used so the avatar replies in chat with a matching voice',
    date_placeholder: 'dd.mm.yyyy',
    public_memorial_hint:
      'A public memorial is reachable by anyone with the direct link, no invite needed — e.g. for well-known people. Private (default) is visible only to the owner and invited people.',
    submit: 'Create Memorial',
    creating: 'Creating…',
    error_create: 'Failed to create memorial',
  },
  media: {
    loading: 'Loading media…',
    title: 'Media files',
    upload: 'Upload file',
    uploading: 'Uploading…',
    empty: 'No media files yet',
    upload_first: 'Upload first file',
    cover_badge: 'Cover',
    remove_cover: 'Remove cover',
    set_cover: 'Set as cover',
    animate: 'Animate photo',
    animating: 'Starting…',
    processing: 'Processing…',
    animated_done: 'Animated',
    animation_ready: 'Animation ready!',
    delete_file_title: 'Delete file',
    upload_error: 'Error uploading file',
    delete_confirm: 'Are you sure you want to delete this file? This action cannot be undone.',
    delete_error: 'Error deleting file',
    error_prefix: 'Error:',
    animation_started: 'Animation started',
    animation_error: 'Error starting animation',
    redis_setup_error: '⚠️ Photo animation requires Redis and a Celery worker.\n\nInstructions:\n1. Start Redis: docker run -d -p 6379:6379 redis\n2. Start the Celery worker: cd backend && celery -A app.workers.worker worker --loglevel=info\n\nSee the docs for details.',
    timeout: 'Timed out',
    waiting_video: 'Waiting for video...',
    status_check_error: 'Error checking status',
  },
  demoTutorial: {
    skip: 'Skip tutorial',
    got_it: 'Got it',
    steps: {
      1: {
        emoji: '👨‍👩‍👧‍👦',
        title: '4 families. 5 generations.',
        text: 'Each family has its own memorials, memories, and AI avatars built from real stories. Click any family to explore.',
        next: "Let's go →",
      },
      2: {
        emoji: '🕯',
        title: 'Meet the family',
        text: 'Each person here has their own memories and an AI avatar. Click anyone to start a conversation.',
        next: 'Got it →',
      },
      3: {
        emoji: '💬',
        title: 'Chat with the avatar',
        text: 'Ask anything — the avatar responds based on real memories. The more memories are added, the richer the conversation becomes.',
        next: 'Got it',
      },
      4: {
        emoji: '📖',
        title: 'Memories power the avatar',
        text: 'Everything the avatar knows comes from these memories. Anyone you share the link with can contribute more.',
        next: 'Got it',
      },
      5: {
        emoji: '🌳',
        title: 'One big family',
        text: 'All these people are connected across generations. Avatars can reference shared memories across multiple memorials.',
        next: 'Got it',
      },
    },
  },
  demoPage: {
    title: 'Explore Memorial Families',
    subtitle: 'Four real families across five generations — Irish, Scottish, Chinese and Italian Australians whose stories are woven together by history. Chat with their AI avatars, read memories, explore the family tree.',
    create_cta: 'Create Your Memorial →',
    signin: 'Sign In',
    all_families: '← All Families',
    loading: 'Loading…',
    memorials_count: '{count} memorials',
    memories_count: '{count} memories',
    alive_suffix: ' (alive)',
    bottom_cta_text: 'Preserve your own family stories — it takes just a few minutes to start.',
    bottom_cta_btn: 'Create Your Free Memorial',
    families: {
      Kelly: {
        label: 'The Kelly Family',
        subtitle: 'Irish-Australian · 5 generations',
        description: 'From the Ballarat goldfields to modern Sydney. Sean Kelly arrived from Cork in 1865 with nothing but hope — his family built a legacy spanning 160 years.',
        patriarch: 'Sean Patrick Kelly',
      },
      Anderson: {
        label: 'The Anderson Family',
        subtitle: 'Scottish-Australian · 4 generations',
        description: 'Duncan Anderson left Scotland in 1862 for a sheep station in the South Australian outback. His daughter Helen bridged two families by marrying James Kelly.',
        patriarch: 'Duncan Alasdair Anderson',
      },
      Chang: {
        label: 'The Chang Family',
        subtitle: 'Chinese-Australian · 5 generations',
        description: "Ah Fong Chang worked the Ballarat goldfields alongside Sean Kelly in 1866. His family wove Chinese-Australian heritage through Sydney's story for generations.",
        patriarch: 'Ah Fong Chang',
      },
      Rossi: {
        label: 'The Rossi Family',
        subtitle: 'Italian-Australian · 3 generations',
        description: "Enzo Rossi sailed from Sicily to Sydney in 1952. His family's construction work shaped the northern suburbs — and their lives intertwined with the Kellys.",
        patriarch: 'Enzo Rossi',
      },
    },
  },
  verifyEmail: {
    default_success: 'Email verified successfully',
    default_error: 'Verification failed. The link may be expired.',
    loading_title: 'Verifying your email…',
    loading_sub: 'Please wait a moment.',
    success_title: 'Email verified!',
    success_sub: 'Your account is now fully activated.',
    go_dashboard: 'Go to Dashboard →',
    error_title: 'Verification failed',
    error_hint_1: 'You can',
    error_hint_link: 'sign in',
    error_hint_2: 'and request a new verification email from the banner in the app.',
    no_token_title: 'Invalid link',
    no_token_sub: 'No verification token found in the URL.',
  },
  errorBoundary: {
    title: 'Something went wrong',
    generic_error: 'An unexpected error occurred.',
    go_home: '← Go to Home',
    reload: 'Reload page',
  },
  verifyBanner: {
    sent: 'Verification email sent! Check your inbox.',
    prompt_1: 'Please verify your email address',
    prompt_2: 'to keep your account secure.',
    sending: 'Sending…',
    resend: 'Resend email',
    dismiss: 'Dismiss',
  },
  hiddenConnections: {
    title: 'Hidden family connections',
    hint: "We'll find relatives across multiple generations — including those who changed their surname or are connected through other families' marriages.",
    searching: 'Searching...',
    refresh: 'Refresh',
    discover: 'Find hidden connections',
    empty: 'No hidden connections found. Add more relatives to the family tree.',
    found_prefix: 'Found',
    connection_singular: 'connection',
    connection_plural: 'connections',
    through: 'spanning',
    generations: 'generations',
    chain_label: 'Relationship chain:',
    hop_labels: {
      2: 'Grandparent by marriage',
      3: 'Cousin',
      4: 'Second cousin',
      5: 'Third cousin',
    },
    hop_label_fallback: 'Distant relative ({hops} steps)',
  },
  tutorial: {
    step_label: 'Step {n} of {total}',
    skip: 'Skip',
    back: 'Back',
    next: 'Next',
    finish: 'Create Memorial →',
    close: 'Close',
    steps: [
      {
        title: 'Create the memorial',
        desc: "Enter the person's full name, birth and death dates (if applicable), and gender — used to pick the avatar's voice. Add a short description or biography if you'd like. That's everything needed for the first step.",
      },
      {
        title: 'Upload a photo',
        desc: "Add one or more portraits. Later, a photo can be animated into a talking digital avatar, and once you upload a voice sample, the avatar can speak in their own voice.",
      },
      {
        title: 'Add memories',
        desc: 'Write down stories, life moments, characteristic phrases and habits. The more memories you add, the more "memory" the avatar has — and the more alive and real it feels in conversation.',
      },
      {
        title: 'Talk to the avatar',
        desc: "Ask the avatar a question — it will answer in this person's voice and style. Important: the avatar only answers based on the memories you've added and never makes facts up — if there isn't enough information, it will say so honestly.",
      },
    ],
  },
  memoryList: {
    title: 'Memories',
    invite_friend: 'Invite a friend',
    invite_loading: '...',
    add_memory: 'Add a memory',
    share_heading: 'Share — let loved ones add their stories about {name}',
    share_heading_no_name: 'Share — invite loved ones to contribute their stories',
    invite_sms:
      "I'm collecting memories about {name}. You can share yours too: {url}",
    anonymous_person: 'this person',
    copy: 'Copy',
    copied: '✓ Copied',
    or_ready_message: 'or send a ready-made message:',
    copy_text: 'Copy text',
    share: 'Share',
    search_placeholder: 'Search memories...',
    found_count: 'Found: {n} memories',
    loading: 'Loading memories…',
    mic_denied: 'Microphone access denied. Allow access in your browser settings.',
    transcribe_error: 'Transcription failed',
    network_error: 'Network error. Make sure the backend is running.',
    load_failed: 'Could not load memories',
    retry: 'Retry',
    submit_error_add: 'Could not add memory',
    submit_error_update: 'Could not update memory',
    delete_error: 'Could not delete memory',
    invite_link_error: 'Could not create link. Please try again.',
    title_optional: 'Title (optional)',
    short_title_ph: 'Short title',
    memory_text_label: 'Memory text *',
    memory_content_ph:
      'Write about this person — their life, character, important moments…',
    voice_hide: 'Hide',
    voice_show: 'Add a voice memory',
    audio_hint:
      'Record your story or upload an audio file — it will be transcribed and added to the memory.',
    record_start: 'Start recording',
    record_stop: 'Stop recording',
    upload_audio: 'Upload audio',
    transcribing: 'Transcribing…',
    transcribe_btn: 'Convert to text',
    delete: 'Remove',
    when_optional: 'When was it (optional)',
    save: 'Save',
    saving: 'Saving…',
    edit_title_label: 'Title',
    edit_content_label: 'Text *',
    edit_when: 'When was it',
    empty_search: 'No results for «{q}»',
    empty_none: 'No memories yet',
    empty_hint: 'Add memories so the AI avatar can answer questions',
    added: 'Added:',
    edit: 'Edit',
    delete_memory: 'Delete',
    confirm_delete: 'Delete this memory?',
    native_share_title: 'Memories · {name}',
  },
  contribute: {
    loading: 'Loading...',
    invalid_link: 'Link is invalid',
    invitation_for: 'Invitation for: {label}',
    subtitle: "You've been invited to share memories about {name}.",
    subtitle2: 'Your words matter — they will be preserved forever.',
    tab_record: '🎙 Record a memory',
    tab_chat: '💬 Chat',
    saved_title: 'Memory saved!',
    saved_hint: 'Thank you! Your words are now part of this memory.',
    record_again: 'Record another',
    viral_share: '💌 Share further — let others tell their story too',
    record_hint: 'Click the button and speak. We will record and save your words.',
    btn_record: 'Click to record',
    btn_stop: 'Stop recording',
    transcribing: 'Transcribing…',
    label_title: 'Title (optional)',
    label_text: 'Memory text',
    placeholder_title: 'E.g. Our last summer',
    placeholder_text: 'Enter or edit the memory text…',
    btn_save_memory: 'Save memory',
    btn_saving: 'Saving…',
    btn_rerecord: 'Re-record',
    or_type: '— or type instead —',
    placeholder_title_manual: 'Title (optional)',
    placeholder_text_manual: 'Write a memory…',
    btn_save: 'Save',
    mic_denied: 'Microphone access denied. Allow access in your browser.',
    transcribe_error: 'Transcription failed. Please type the memory manually.',
    clipboard_copied: 'Text copied to clipboard!',
    save_error: 'Failed to save memory. Please try again.',
    viral_name_fallback: 'this person',
    viral_text: 'I shared memories about {name}. Share yours too: {url}',
    viral_native_title: 'Memories of {name}',
  },
  common: {
    save: 'Save',
    cancel: 'Cancel',
    loading: 'Loading...',
    saving: 'Saving...',
  },
  family: {
    deceased: 'Deceased',
    unknown: 'Unknown',
    // relation types
    parent: 'Parent',
    child: 'Child',
    spouse: 'Spouse',
    sibling: 'Sibling',
    step_parent: 'Stepparent',
    step_child: 'Stepchild',
    adoptive_parent: 'Adoptive parent',
    adoptive_child: 'Adopted child',
    half_sibling: 'Half-sibling',
    partner: 'Partner (domestic)',
    ex_spouse: 'Ex-spouse',
    custom: 'Other',
    // header & buttons
    title: 'Family Tree',
    people_count: (n) => `${n} people`,
    loading: 'Loading family tree...',
    hidden_connections: 'Hidden connections',
    hide_connections: 'Hide connections',
    add_relation: '+ Add relation',
    edit_mode: '✎ Edit layout',
    edit_mode_exit: '✓ Done editing',
    connect_title: 'Create connection',
    kelly_only_banner:
      'Showing the Kelly family only — other families are hidden until we connect them in the next steps.',
    kelly_anderson_banner:
      'Showing the Kelly and Anderson families. Other surnames (e.g. Chang, Rossi) are hidden in this demo view.',
    kelly_anderson_third_banner:
      'Showing Kelly, Anderson, and one additional family branch. Other surnames stay hidden to keep the tree readable.',
    kelly_anderson_four_banner:
      'Showing four families: Kelly, Anderson, and two additional branches. Other surnames stay hidden for readability.',
    unlock_hint: 'Connected families:',
    stub_show: 'Show tree',
    stub_show_family_title: 'Show this family tree',
    // empty state
    empty_title: 'Your family history starts here',
    empty_hint: 'Add the first relation to start building the tree',
    // legend
    legend_spouses: 'Spouses',
    legend_siblings: 'Siblings',
    legend_union: 'Union with children',
    legend_cross_family: 'Cross-family marriage',
    legend_cross_family_short: 'cross-family marriage',
    marriage_rings_title: 'Marriage (rings appear when zoomed in)',
    legend_hidden: 'Hidden connection',
    // relations list
    relations_title: 'Relations',
    delete: 'Delete',
    delete_confirm: 'Delete this relation?',
    add_error: 'Error adding relation',
    error: 'Error',
    // add form
    form_memorial: 'Memorial *',
    form_select_placeholder: '— select a memorial —',
    form_id_placeholder: 'Memorial ID',
    form_type: 'Relation type *',
    form_notes: 'Notes (optional)',
    form_notes_placeholder: 'Additional information',
    // form option groups
    group_parent_child: 'Parent / Child',
    group_partner: 'Partner / Spouse',
    group_sibling: 'Siblings',
    group_other: 'Other',
    // type descriptions
    type_parent_desc: 'Biological parent',
    type_child_desc: 'Biological child',
    type_adoptive_parent_desc: 'Adoptive parent',
    type_adoptive_child_desc: 'Adopted child',
    type_step_parent_desc: 'Stepparent',
    type_step_child_desc: 'Stepchild',
    type_spouse_desc: 'Spouse (married)',
    type_partner_desc: 'Partner (domestic partnership)',
    type_ex_spouse_desc: 'Ex-spouse (divorced)',
    type_sibling_desc: 'Full sibling',
    type_half_sibling_desc: 'Half-sibling (one shared parent)',
    type_custom_desc: 'Other (specify manually)',
    form_custom_label: 'Relation name',
    form_custom_label_placeholder: 'e.g. Godfather, Guardian...',
    add_submit: 'Add relation',
    // relationship tooltip labels
    rel_parent: 'Parent',
    rel_child: 'Child',
    rel_spouse: 'Spouse',
    rel_ex_spouse: 'Ex-spouse',
    rel_sibling: 'Sibling',
    rel_grandparent: 'Grandparent',
    rel_grandchild: 'Grandchild',
    rel_aunt_uncle: 'Aunt / Uncle',
    rel_niece_nephew: 'Niece / Nephew',
    rel_parent_in_law: 'Parent-in-law',
    rel_stepparent: 'Stepparent',
    rel_child_in_law: 'Child-in-law',
    rel_stepchild: 'Stepchild',
    rel_great_grandparent: 'Great-grandparent',
    rel_great_grandchild: 'Great-grandchild',
    rel_great_great_grandparent: 'Great-great-grandparent',
    rel_great_aunt_uncle: 'Great-aunt / Great-uncle',
    rel_cousin: 'Cousin',
    rel_distant_prefix: 'Relative',
    // hints
    tree_layout_hint:
      'Each card is one memorial. Older generations are at the top, younger at the bottom. The orange line marks a marriage between two separate family lines.',
    tree_controls:
      'Wheel — zoom · Drag — pan · Click — open profile · Hover — relationship to the selected person',
    fit_whole_tree: 'Fit whole tree',
    center_on_person: 'Center on this person',
    fullscreen: 'Fullscreen',
    exit_fullscreen: 'Exit fullscreen',
    layout_generations: 'Generations (demo)',
    layout_pedigree: 'Pedigree layout',
    cluster_other: 'Other',
    gen_legend_hint:
      'Each row is one generation — grandparents at top, grandchildren at bottom. Each color represents a family name. Lines connect parents and children; horizontal lines connect spouses.',
    gen_legend_expand: 'How to read the tree',
    split_border_title:
      'Split border: each half is a surname-cluster color from the legend (e.g. Kelly vs Anderson)—not a separate "missing" family line on the graph.',
    gen_legend_help_borders:
      'Card border color = family name color. A split two-color border means this person connects two families (e.g. born Kelly, married into Anderson). A gold glow means they married into a different family name shown in the tree.',
    gen_legend_help_lines:
      'Lines going down = parent → child. Horizontal line between two portraits = spouses. A dashed golden line = marriage between two different family names. Zoom in to see wedding rings — each ring uses the spouse\'s family color.',
    gen_legend_help_generation:
      'Row position is based on the family link path, not birth year — so someone older can appear lower if the data connects them as a descendant of your starting person.',
    gen_legend_help_labels:
      'The small italic line under a name shows how that person is related to {rootName}. "Distant relative via…" means the connection is more than two steps away.',
    gen_legend_root_fallback: 'the person on this memorial page',
    gen_row_label: (n) => `Gen ${n}`,
  },
}
